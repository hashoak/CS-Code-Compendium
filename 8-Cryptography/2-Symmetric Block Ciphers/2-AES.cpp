...
void KeyExpansion(unsigned char inputKey[16],unsigned char expandedKeys[176]) {
	...
	while (bytesGenerated<176) {
		for (int i=0;i<4;i++) tmpCore[i]=expandedKeys[i+bytesGenerated-4];
		if (bytesGenerated%16==0) {
            ...
            tmpCore[2]=tmpCore[3];
            tmpCore[3]=t;
            tmpCore[0]=s[tmpCore[0]];
            tmpCore[1]=s[tmpCore[1]];
            tmpCore[2]=s[tmpCore[2]];
            tmpCore[3]=s[tmpCore[3]];
            tmpCore[0]^=rcon[rconIteration++];
        }
...
void AESEncrypt(unsigned char * message,unsigned char * expandedKey,unsigned char * encryptedMessage) {
	...
	for (int i=0;i<numberOfRounds;i++) Round(state,expandedKey+(16*(i+1)));
...
void AESDecrypt(unsigned char * encryptedMessage,unsigned char * expandedKey,unsigned char * decryptedMessage) {
	...
	int numberOfRounds=9;
	for (int i=8;i >= 0;i--) InverseRound(state,expandedKey+(16*(i+1)));
...
int main() {
	...
	int i=0;
	unsigned int c;
	while (hex_chars_stream>>hex>>c) key[i]=c,i++;
	...
	for (int i=0;i<paddedMessageLen;i+=16) AESEncrypt(paddedMessage+i,expandedKey,encryptedMessage+i);

	cout<<"Encrypted message in hex:"<<endl;
	for (int i=0;i<paddedMessageLen;i++) cout<<hex<<(int) encryptedMessage[i]<<" ";
	cout<<endl;

    int messageLen=strlen((const char *)encryptedMessage);
	unsigned char * decryptedMessage=new unsigned char[messageLen];
	for (int i=0;i<messageLen;i+=16) AESDecrypt(encryptedMessage+i,expandedKey,decryptedMessage+i);

	cout<<"Decrypted message in hex:"<<endl;
	for (int i=0;i<messageLen;i++) cout<<hex<<(int)decryptedMessage[i]<<" ";
	cout<<endl;
